...
# note need to change to using pad_to_jet rather than max jet
filepath = sys.argv[1]
outfile  = sys.argv[2]
pad_to_jet = 20
...
def quick_method(jets, particles, max_jet):
    ...
    for i in range(max_jet):
        delta_Rs = jets[:,i].deltaR(particles)  
...
def expensive_method(jets, particles, max_jet):
    ...
    for event, (event_jets, event_particles) in tqdm(enumerate(zip(jets, particles))):
        ...
        for i in range(p_status_len):
            ...
            mins[arg//p_status_len] = np.where(all_Rs[arg//p_status_len, arg%p_status_len]<0.4, new_values[arg%p_status_len], 0)
